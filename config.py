...
class Config:
    ...
    MINIMAX_MODEL: str = "abab6.5s-chat"  # Minimax 2.1
    
    # Ollama Configuration (if using local proxy)
    OLLAMA_HOST: str = os.getenv("OLLAMA_HOST", "http://localhost:11434")
    OLLAMA_MODEL: str = "minimax:latest"
    ...
    def validate(cls) -> bool:
        ...
        if not cls.USE_OLLAMA and (not cls.MINIMAX_API_KEY or not cls.MINIMAX_GROUP_ID):
            raise ValueError("MINIMAX_API_KEY and MINIMAX_GROUP_ID must be set if not using Ollama")
        return True
